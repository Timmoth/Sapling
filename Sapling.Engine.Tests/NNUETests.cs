...
public class NNUETests
{
    ...
    [InlineData(
        ...
        "5k2/1p3P2/6p1/3p4/p6p/6r1/1P3n2/5n1K w - - 6 87")]
    [InlineData(
        ...
        "4R3/8/8/8/3n1p2/3r4/1K1k4/8 w - - 16 139")]
    public void ApplyMatchesFen(string moves, string fen)
    {
        ...
        // Then
        board.Data.ToFen().Should().Be(fen);
    }


    public unsafe void Apply(BoardState board, string[] moves, int moveIndex)
    {
        ...
        var oldEnpassant = board.Data.EnPassantFile;
        var prevCastleRights = board.Data.CastleRights;


        var validMoves = new List<uint>();
        board.Data.GenerateLegalMoves(validMoves, false);
        var moveString = moves[moveIndex];

        var move = validMoves.FirstOrDefault(m => m.ToUciMoveName() == moveString);
        board.Data.PartialApply(move);
        board.Data.UpdateCheckStatus();
        var hashHistory = stackalloc ulong[100];
        board.Data.FinishApply(board.WhiteAccumulator, board.BlackAccumulator, hashHistory, move, oldEnpassant, prevCastleRights);
