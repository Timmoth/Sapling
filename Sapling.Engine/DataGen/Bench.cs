...
namespace Sapling.Engine.DataGen
{
    ...
    {
        private static readonly string[] BenchFens = new string[]
        {
        ...
        public static void Run(int depth = 16)
        {
            var transpositionSize = (int)TranspositionTableExtensions.CalculateTranspositionTableSize(256);
            var transpositions = GC.AllocateArray<Transposition>(transpositionSize, true);

            var searcher = new Searcher(transpositions);

            var stopwatch = new Stopwatch();
            long totalNodes = 0;
            long totalTime = 0;
            foreach (var fen in BenchFens)
            {
            ...
            Console.WriteLine($"info nodes {totalNodes} time {(int)totalTime}ms {(totalNodes / totalTime)}knps");
        }
    }
}
